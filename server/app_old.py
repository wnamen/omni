...
async def health_check():
    ...
    if models_loaded:
        try:
            # For YOLO: check device
            _ = next(yolo_model.parameters()).device
            # For Florence-2: check processor/model device
            _ = next(caption_model_processor['model'].model.parameters()).device
        except Exception:
            model_ok = False
...
async def parse(request: ImageRequest):
    ...
    logger.info(f"🔥 PARSE ENDPOINT CALLED at {start_timestamp} 🔥")
    ...
    try:
        ...
        logger.info(f"🖼️ [{start_timestamp}] Original image loaded: {image.size} pixels")
        ...
        try:
            ...
            logger.info(f"[{start_timestamp}] OCR completed in {ocr_time:.2f} seconds")
            ...
            logger.info(f"[{start_timestamp}] Parsing completed in {parsing_time:.2f} seconds")
