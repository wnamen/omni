...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    if model_name == "blip2":
    ...
    elif model_name == "microsoft/Florence-2-base" or model_name == "florence2":
        ...
        if device == 'cpu':
            ...
            model = AutoModelForCausalLM.from_pretrained("microsoft/Florence-2-base", torch_dtype=torch.float16, trust_remote_code=True).to(device)
    else:
        raise ValueError(f"Unsupported model_name: {model_name}")
    
    if model is None:
        raise ValueError(f"Failed to load model for {model_name}")
